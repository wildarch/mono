...
This lets us express compilation as a recursive function, as shown in figure~\ref{fig:strict}.

\begin{figure*}[h]
    \begin{equation*}
        \begin{array}{l l @{}l l}
            \strict & z         & {}= 1 \models I                                                         &                                                                                  \\
            \strict & s e       & {}= n+1 \models (0 \models K) \coprod (n \models c)                     & \text{where } (n \models c) := \strict e                                         \\
            \strict & \lambda e & {}= (0 \models K c)                                                     & \text{where } (0 \models c) := \strict e                                         \\
            \strict & \lambda e & {}= (n-1 \models K c)                                                   & \text{where } (n \models c) := \strict e                                         \\
            \strict & e_1 \ e_2 & {}= (\max n_1 n_2 \models ((n_1 \models c_1) \coprod (n_2 \models c_2)) & \text{where } (n_1 \models c_1) := \strict e_1; (n_2 \models c_2) := \strict e_2 \\
        \end{array}
    \end{equation*}
    \caption{Compilation function for the strict algorithm. We have reused $\models$ from the paper. In code one may represent it as a tuple, e.g. $n \models e$ becomes $(n, e)$.}
    \label{fig:strict}
\end{figure*}
