...
As this is core to the algorithm, we adopt the same approach for our algorithm descriptions in Section~\ref{sec:kiselyov}.
However, in the example expressions we give in this paper, we continue to use the easier-to-read named bindings.

%The lambda ($\lambda$) calculus is perhaps the smallest programming language.
%At its core there is only the definition of functions (\emph{abstraction}) and the \emph{application} of a function to an argument.
%We use the $\lambda$ symbol to denote abstraction, e.g. $\lambda x.x$ defines a function (unnamed) that takes an argument $x$ (the $x$ before the dot), and when applied to an $y$ returns that same value $y$.
%This application may be written as $(\lambda x.x) y$.
%$\lambda x.x$ is in fact the definition of the \emph{identity} function: a function that returns its argument unchanged.
%we can nest abstractions to create functions that accept multiple arguments, for example $\lambda x.\lambda y. x$, a function that takes two arguments and returns the first (the second is discarded).
%
%Functions may be arguments to other functions: $(\lambda x.x) (\lambda x.x)$ is a valid expression in the lambda calculus (it evaluates to $\lambda x.x$).
%
%TODO: Outermost reduction
...
The core idea is to define a set of basic functions, or \emph{combinators}, that can be combined to pass variables to the proper subexpressions.
...
Another possibility is that parts of the graph are still reduceable, but our reduction strategy does not `reach' it.
...
All other code is written in Rust and compiled using the optimizing \texttt{release} profile (\texttt{rustc} version 1.65.0).
...
We used a recently proposed semantic translation algorithm to compile lambda calculus programs into smaller combinator expressions~\cite{kiselyov_lambda_2018} and have implemented modern combinator graph reduction engines based on the designs of Miranda~\cite{turner_new_1979} and TIGRE~\cite{koopman_fresh_1989}.
