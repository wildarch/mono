...
\begin{equation*}
    \begin{array}{l l l l}
        ...
        \lazyeta & \lambda e & = & (\emptyset \models K) \coprod (\Gamma \models c)                        \\
                 &           &   & \text{where } (\Gamma,f \models c) := \lazyeta e                        \\
        \lazyeta & \lambda e & = & \Gamma \models K \; c                                                   \\
                 &           &   & \text{where } (\Gamma,t \models c) := \lazyeta e                        \\
        \\
        \lazyeta & e_1 \ e_2 & = & (\Gamma \models ((\Gamma_1 \models c_1) \coprod (\Gamma_2 \models c_2)) \\
...
\textbf{TODO: } Check this, possibly need to take (C d, V) = N (C d)
The context merge function $\sqcup$ is unchanged, but we do have some new cases for the semantic function:

\begin{equation*}
    \arraycolsep=1.4pt
    \begin{array}{l l l l l}
        % Added for eta optimization
        (\Gamma_1,f \models c_1) & \coprod & (\emptyset \models V)    & = & c_1                                                      \\
        (\emptyset \models V)    & \coprod & (\Gamma_2,f \models c_2) & = & (\emptyset \models CI) \coprod (\Gamma_2 \models c_2)    \\

        (\Gamma_1,t \models c_1) & \coprod & (\emptyset \models V)    & = & (\Gamma_1 \models (s \coprod l)) \coprod i               \\
                                 &         &                          &   & \text{where } l := (\Gamma_1 \models c_1 )               \\
                                 &         &                          &   & \text{where } s := (\emptyset \models S )                \\
                                 &         &                          &   & \text{where } i := (\emptyset \models I )                \\
        (\emptyset \models V)    & \coprod & (\Gamma_2,t \models c_2) & = & (\emptyset \models SI) \coprod (\Gamma_2 \models c_2)    \\
        (\emptyset \models c_1)  & \coprod & (\emptyset \models V)    & = & c_1                                                      \\
        (\emptyset \models V)    & \coprod & (\emptyset \models c_2)  & = & CI c_2                                                   \\
        \\
